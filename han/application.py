...
def a_class_cluster_feature_setting2():
	a_embed = np.around(np.random.normal(0, 0.01, [args.A_n, args.embed_d]), 4)
	embed_f = open(args.data_path + "node_embedding.txt", "r")
	for line in islice(embed_f, 0, None):
		line = line.strip()
...
#macro, micro = NC.model(train_num, test_num)
#nmi, ari = NCL.model(cluster_id)
...
for i in range(10):
	print(str(i))	
	train_num, test_num = a_a_collab_feature_setting() #setup of author-author collaboration prediction task
	AUC_t, F1_t=LP.model(train_num, test_num)
	AUC.append(AUC_t)
	F1.append(F1_t)
print('AUC max:%.4f mean:%.4f std:%.4f'%(np.array(AUC).max(),np.array(AUC).mean(),np.array(AUC).std()))
print('F1 max:%.4f mean:%.4f std:%.4f'%(np.array(F1).max(),np.array(F1).mean(),np.array(F1).std()))
print("------author collaboration link prediction end------")
...
#print("------venue recommendation------")
#recall_all, pre_all, F_1=[],[],[]
#for i in range(10):
	#print(str(i))	
	#recall_all_t, pre_all_t, F_1_t=a_v_recommendation()
	
	#recall_all.append(recall_all_t)
	#pre_all.append(pre_all_t)
	#F_1.append(F_1_t)
#print('recall_all max:%.4f mean:%.4f std:%.4f'%(np.array(recall_all).max(),np.array(recall_all).mean(),np.array(recall_all).std()))
#print('pre_all max:%.4f mean:%.4f std:%.4f'%(np.array(pre_all).max(),np.array(pre_all).mean(),np.array(pre_all).std()))
#print('F_1 max:%.4f mean:%.4f std:%.4f'%(np.array(F_1).max(),np.array(F_1).mean(),np.array(F_1).std()))
#print("------venue recommendation end------")


print("------author classification/clustering 0.1------")
macro, micro=[],[]
for i in range(10):
	print(str(i))	
	train_num, test_num, cluster_id = a_class_cluster_feature_setting() #setup of author classification/clustering task
	macro_t, micro_t = NC.model(train_num, test_num)
	
	macro.append(macro_t)
	micro.append(micro_t)

print('macro max:%.4f mean:%.4f std:%.4f'%(np.array(macro).max(),np.array(macro).mean(),np.array(macro).std()))
print('micro max:%.4f mean:%.4f std:%.4f'%(np.array(micro).max(),np.array(micro).mean(),np.array(micro).std()))

print("------author classification/clustering end------")

print("------author classification/clustering 0.3------")
macro, micro, nmi, ari  =[],[],[],[]
for i in range(10):
	print(str(i))	
	train_num, test_num, cluster_id = a_class_cluster_feature_setting2() #setup of author classification/clustering task
	macro_t, micro_t = NC.model(train_num, test_num)
	nmi_t, ari_t = NCL.model(cluster_id)	
...
print('macro max:%.4f mean:%.4f std:%.4f'%(np.array(macro).max(),np.array(macro).mean(),np.array(macro).std()))
print('micro max:%.4f mean:%.4f std:%.4f'%(np.array(micro).max(),np.array(micro).mean(),np.array(micro).std()))
print('nmi max:%.4f mean:%.4f std:%.4f'%(np.array(nmi).max(),np.array(nmi).mean(),np.array(nmi).std()))
print('ari max:%.4f mean:%.4f std:%.4f'%(np.array(ari).max(),np.array(ari).mean(),np.array(ari).std()))
print("------author classification/clustering end------")
