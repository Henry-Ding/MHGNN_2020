...
class HetAgg(nn.Module):
	def __init__(self, args, feature_list, a_neigh_list_train, p_neigh_list_train, v_neigh_list_train,\
		...
		#self.a_neigh_att2 = nn.Parameter(torch.ones(embed_d * 2, 1), requires_grad = True)
		#self.p_neigh_att2 = nn.Parameter(torch.ones(embed_d * 2, 1), requires_grad = True)
		#self.v_neigh_att2 = nn.Parameter(torch.ones(embed_d * 2, 1), requires_grad = True)	
		
		self.a_neigh_att = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.p_neigh_att = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.v_neigh_att = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
	
		self.a_neigh_att2 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.p_neigh_att2 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.v_neigh_att2 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)	
		
		self.a_neigh_att3 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.p_neigh_att3 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.v_neigh_att3 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)	
		
		self.a_neigh_att4 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.p_neigh_att4 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)
		self.v_neigh_att4 = nn.Parameter(nn.init.xavier_normal_(torch.Tensor(embed_d * 2, 1)), requires_grad = True)		
		...
		self.linear = nn.Linear(in_f_d, embed_d)
	...
	def a_content_agg(self, id_batch): #heterogeneous content aggregation
		...
		a_net_embed_batch = self.feature_list[6][id_batch][:,:embed_d]
	...
	def p_content_agg(self, id_batch):
		...
		p_a_embed_batch = self.feature_list[0][id_batch][:,:embed_d]
		p_t_embed_batch = self.feature_list[1][id_batch][:,:embed_d]		
		...
		p_v_net_embed_batch = self.feature_list[2][id_batch][:,:embed_d]
		p_a_net_embed_batch = self.feature_list[3][id_batch][:,:embed_d]
		p_net_embed_batch = self.feature_list[5][id_batch][:,:embed_d]
	...
	def v_content_agg(self, id_batch):
		embed_d = self.embed_d
		v_net_embed_batch = self.feature_list[8][id_batch][:,:embed_d]
		...
		v_text_embed_batch_5 = self.feature_list[9][id_batch, 4 * embed_d: 5 * embed_d][0]
	...
	def node_het_agg(self, id_batch, node_type): #heterogeneous neighbor aggregation
		...
		if node_type == 1:
			...
			atten_w2 = self.act(torch.bmm(concate_embed, self.a_neigh_att2.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.a_neigh_att2.size())))	
			atten_w3 = self.act(torch.bmm(concate_embed, self.a_neigh_att3.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.a_neigh_att3.size())))		
			atten_w4 = self.act(torch.bmm(concate_embed, self.a_neigh_att4.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.a_neigh_att4.size())))			
		elif node_type == 2:
			atten_w = self.act(torch.bmm(concate_embed, self.p_neigh_att.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.p_neigh_att.size())))
			atten_w2 = self.act(torch.bmm(concate_embed, self.p_neigh_att2.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.p_neigh_att2.size())))	
			atten_w3 = self.act(torch.bmm(concate_embed, self.p_neigh_att3.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.p_neigh_att3.size())))		
			atten_w4 = self.act(torch.bmm(concate_embed, self.p_neigh_att4.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.p_neigh_att4.size())))				
		else:
			atten_w = self.act(torch.bmm(concate_embed, self.v_neigh_att.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.v_neigh_att.size())))
			atten_w2 = self.act(torch.bmm(concate_embed, self.v_neigh_att2.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.v_neigh_att2.size())))	
			atten_w3 = self.act(torch.bmm(concate_embed, self.v_neigh_att3.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.v_neigh_att3.size())))			
			atten_w4 = self.act(torch.bmm(concate_embed, self.v_neigh_att4.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.v_neigh_att4.size())))				
		atten_w = self.softmax(atten_w).view(len(c_agg_batch), 1, 4)
		atten_w2 = self.softmax(atten_w2).view(len(c_agg_batch), 1, 4)
		atten_w3 = self.softmax(atten_w3).view(len(c_agg_batch), 1, 4)
		atten_w4 = self.softmax(atten_w4).view(len(c_agg_batch), 1, 4)
		...
		weight_agg_batch2 = torch.bmm(atten_w2, concate_embed).view(len(c_agg_batch), self.embed_d)
		weight_agg_batch3 = torch.bmm(atten_w3, concate_embed).view(len(c_agg_batch), self.embed_d)
		weight_agg_batch4 = torch.bmm(atten_w4, concate_embed).view(len(c_agg_batch), self.embed_d)
		
		#weight_agg_batch=torch.cat((weight_agg_batch, weight_agg_batch2),1).view(len(weight_agg_batch), self.embed_d * 2)
		weight_agg_batch_avg=(weight_agg_batch+weight_agg_batch2+weight_agg_batch3+weight_agg_batch4)/4
		#weight_agg_batch_avg=(weight_agg_batch+weight_agg_batch2+weight_agg_batch3)/3
		#weight_agg_batch_avg=(weight_agg_batch+weight_agg_batch2)/2
		#weight_agg_batch_avg = weight_agg_batch
		
		return weight_agg_batch_avg
...
def meta_path_count(mpath, num, *relation):
	start = time.clock()	
	meta_list = [[] for k in range(num[0])]
	if(len(num)==2)	:
		for i in range(num[0]):
			if(relation[0][i]):
				for _, link1 in enumerate(relation[0][i]):			
					if(relation[1][int(link1[1:])]):
						for _, link2 in enumerate(relation[1][int(link1[1:])]):
							if(link2[1:]!=str(i) and (link2 not in meta_list[i])):
								meta_list[i].append(link2)
	elif(len(num)==3):
		for i in range(num[0]):
			if(relation[0][i]):
				for _, link1 in enumerate(relation[0][i]):			
					if(relation[1][int(link1[1:])]):
						for _, link2 in enumerate(relation[1][int(link1[1:])]):			
							if(relation[2][int(link2[1:])]):						
								for _, link3 in enumerate(relation[2][int(link2[1:])]):
									if(link3[1:]!=str(i) and (link3 not in meta_list[i])):
										meta_list[i].append(link3)		
				...
				for _, link1 in enumerate(relation[0][i]):			
					if(relation[1][int(link1[1:])]):
						for _, link2 in enumerate(relation[1][int(link1[1:])]):			
							if(relation[2][int(link2[1:])]):		
								for _, link3 in enumerate(relation[2][int(link2[1:])]):			
									if(relation[3][int(link3[1:])] and link3!=link1):#APVPA P!=P								
										for _, link4 in enumerate(relation[3][int(link3[1:])]):
											if(link4[1:]!=str(i) and (link4 not in meta_list[i])):#APVPA A!=A
												meta_list[i].append(link4)		

	end = time.clock()	
	print('Path %s finish: %.2fs'%(mpath, (end-start)))



	return meta_list

#def meta_path_count(mpath, num, *relation):#目标节点重复计数
	#start = time.clock()	
	#meta_list = [[] for k in range(num[0])]
	#if(len(num)==2)	:
		#for i in range(num[0]):
